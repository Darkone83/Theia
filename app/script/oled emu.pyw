...
class LCDWindow(QMainWindow):
    def __init__(self):
        ...
        root_v.setContentsMargins(16, 16, 16, 12)
        root_v.setSpacing(6)

        # --- Centered LCD pane (no borders/frames) ---
        root_v.addStretch(1)
        ...
        # 4 LCD row labels (fixed 20 chars), no frames
        ...
        for _ in range(4):
            ...
            lbl.setMinimumWidth(20 * 16)  # rough width; font dependent
            lbl.setFrameShape(QFrame.NoFrame)
            lbl.setLineWidth(0)
            lbl.setAttribute(Qt.WA_StyledBackground, True)
            self.row_labels.append(lbl)
            lcd_v.addWidget(lbl, 0, Qt.AlignmentFlag.AlignHCenter)

        center_h = QHBoxLayout()
        center_h.addStretch(1)
        center_h.addWidget(self.lcd_container, 0, Qt.AlignmentFlag.AlignVCenter)
        center_h.addStretch(1)
